...
### تصمیم‌های کاربر (۲۰۲۶-۰۹-۲۲)

۱. نسخهٔ ۱.۹.۲۳ تأیید و کامیت شد (کامیت `e783f9e`).
۲. آنسامبل کامل با PyTorch — گروه وابستگی `ml` تعریف شد.
۳. داده فقط کریپتو داخلی؛ رویدادها دستی — هیچ منبع خارجی ماکرو اضافه نمی‌شود.

### Phase 1 + جریان موازی «همیشه آنلاین» (۲۰۲۶-۰۹-۲۲)

- **Implemented**:
  1. **موتور کیفیت داده** `market/quality.py`: بازرسی تکراری/شکاف/بی‌ترتیبی/عدم تراز/آینده/OHLC نامعتبر/جهش مشکوک + `clean_candles` محافظه‌کار (حذف فقط ردیف‌های به‌صراحت نامعتبر؛ شکاف هرگز پر نمی‌شود).
  2. **خزانهٔ فیچر** `signals/prediction/features.py`: ۱۱ فیچر پایه (بازده، تغییر حجم، RSI، MACD سه‌تایی، ATR، ATR٪، فاصلهٔ نرمال EMA بر حسب ATR، پهنای بولینگر، ADX) با مهر زمانی «بسته‌شدن کندل»؛ اندیکاتورها از موتور موجود می‌آیند (صفر بازنویسی)؛ فیچرهای خارجی از پارامتر `extra`؛ `matrix()` ردیف ناقص را حذف می‌کند نه پر کردن.
  3. **نگهبان اتصال** `market/resilience.py`: تشخیص مرگ وظیفهٔ فید (نه فقط پرچم)، احیا با عقب‌نشینی نمایی (۱s→۱۲۰s) + لرزش ±۲۰٪، ماشین حالت سه‌حاله (online/degraded/offline)، انتشار رویداد `system.connectivity_changed`.
  4. **اتصال به محصول**: `LivePriceFeed.poll_alive` عمومی شد؛ `main_controller` نگهبان را می‌سازد/متوقف می‌کند؛ نشانگر اتصال سه‌حالته شد (کلید موجود `common.reconnecting` برای حالت میانی — بدون رشتهٔ هاردکد).
- **Modified Files**: `app/core/events.py` (+۱ عضو enum)، `market/live_feed.py` (+ویژگی poll_alive)، `ui/controllers/main_controller.py` (ساخت/توقف/نمایش نگهبان)، `pyproject.toml` (+گروه ml، نسخه ۱.۱۰.۰)، `app/core/constants.py`، `BUILD_INFO.txt`، `README.md`، `README_EN.md` (نسخه).
- **New Files**: `market/quality.py`، `market/resilience.py`، `signals/prediction/__init__.py`، `signals/prediction/features.py`، `tests/test_predictive_phase1.py` (۲۴ آزمون)، `tests/test_market_resilience.py` (۱۲ آزمون).
- **Database Changes**: ندارد.
- **Dependencies**: گروه اختیاری `ml = torch/xgboost/lightgbm` (نصب‌شده در محیط توسعه برای فاز ۵).
- **Tests**: ۳۶ آزمون جدید (۲۴ کیفیت/فیچر + ۱۲ نگهبان اتصال)، همه سبز. مجموعهٔ کامل: **۲۰۸۶ قبول / ۰ مردود / ۰ خطا · ۷۹ فایل · ۲۷۲ ثانیه** (۲۰۵۰ + ۳۶).
- **Performance**: کیفیت ۱۰۰۰ کندل ~ میلی‌ثانیه؛ فیچر ۱۲۰ کندل ~ چند ده میلی‌ثانیه (با کش اندیکاتور موجود).
- **Known Issues**: وضعیت «degraded» فعلاً فقط در نوار وضعیت دیده می‌شود؛ کارت تاریخچهٔ اتصال در داشبورد پیش‌بینی (فاز ۱۲) ساخته می‌شود. اجرای واقعی احیا فقط روی ویندوزِ کاربر با قطعی واقعی قابل مشاهده است.
- **Next Phase**: ۲ — موتور فیچر چندتایم‌فریمی (نردبان ۱۳ افق با فعال/غیرفعال شدن خودکار).
